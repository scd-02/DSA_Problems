public class Maths_StepsToDestination {
  static int minSteps(int d) {
    ...
    // to reach d steps we need atmost 2 * d steps as taking i negative step and (i
    // + 1)positive step
    // makes only one step e.g. -i + (i + 1) = 1
    // so for moving 1 step we need 2 steps one negative, and next one positive
    // hence 2 * d steps
    return 2 * d;
  }
}
